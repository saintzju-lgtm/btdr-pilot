...
def get_btdr_dynamic_engine():
    tk = yf.Ticker("BTDR")
    info = tk.info
    
    # 💎 动态获取流通股本 (不再锁定 1.18 亿)
    # 优先取 floatShares，若无则取 shares(总股本)，最次取历史基数 118,500,000
...
# --- 2. AI 审计引擎 (集成期权雷达、庄家意图、结构化分析) ---
@st.cache_data(ttl=3600, show_spinner=False)
def get_ai_pro_audit(p_curr, p_ma5, turnover, p_low, inst_cost, iv, pcr):
    ...
    try:
        ...
        【审计要求】：
        ## 1. 空间定位
        分析现价与 MA5、回归下限(${p_low:.2f})的关系。加粗高亮关键支撑偏离度 {dev_sup:.2f}%。
        ## 2. 量价审计与庄家意图
        分析当前换手率背景下的庄家意图：是洗盘吸筹还是破位出货？结合机构成本 ${inst_cost:.2f} 研判。
        ## 3. 期权雷达分析
        解读 IV({iv:.2%}) 与 PCR({pcr:.2f}) 反映的市场情绪。
        ## 4. 核心研判结论
        明确给出操作建议（**买入/观望/减仓**）及止损位。
        
        文字必须为黑色，重点内容用 **加粗** 高亮。
        """
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[{"role": "system", "content": "实战严密逻辑，禁止幻觉。"}, {"role": "user", "content": prompt}]
        )
...
try:
    hist_df, dynamic_float, reg, rt_v, inst_cost, iv, pcr = get_btdr_dynamic_engine()
    ...
    report = get_ai_pro_audit(curr_p, last_h['MA5'], today_to, p_l_mid, inst_cost, iv, pcr)
    st.markdown(report)
