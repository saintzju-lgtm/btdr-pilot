...
def check_password():
    ...
    if not st.session_state.password_correct:
        st.set_page_config(layout="wide", page_title="终端授权", page_icon="🔒")
...
def get_btdr_data_pro():
    ...
    if rt_vol == 0 and not live_df.empty: rt_vol = live_df['Volume'].sum()
...
def get_ai_audit_pro(p_curr, p_ma5, turnover_str, p_low, p_high):
    ...
    try:
        ...
        prompt = f"""
        你是一名严格的量化审计师。根据以下参数审计 BTDR：
        现价 ${p_curr:.2f}, MA5 ${p_ma5:.2f}, 支撑偏离 {dev_sup:.2f}%, 实时换手率 {turnover_str}。
        
        【规则】：
        1. 必须保留详细逻辑，严禁幻觉（禁止说换手率68%等）。
        2. 结构：## 1. 空间定位 | ## 2. 量价审计 | ## 3. 核心结论（加粗动作+止损位）。
        3. 最后一行必须输出: SCORES: 动能=X, 支撑=X, 换手=X, 趋势=X
        """
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[{"role": "system", "content": "你只讲实战逻辑。"}, {"role": "user", "content": prompt}]
        )
        full_text = response.choices[0].message.content
        
        # --- 增强的评分提取逻辑 (防止 index out of range) ---
        scores = [50, 50, 50, 50]
        score_match = re.search(r"SCORES:.*", full_text)
        if score_match:
            nums = re.findall(r"\d+", score_match.group())
            if len(nums) >= 4:
                scores = [int(n) for n in nums[:4]]
        
        clean_text = re.sub(r"SCORES:.*", "", full_text).strip()
        return clean_text, scores
    except Exception as e:
        return f"❌ 审计调用异常: {str(e)}", [0, 0, 0, 0]
...
st.markdown("""
    ...
    .stMarkdown, p, li, h2, h3, div, span { color: #000000 !important; }
    ...
    div.stButton > button {
        ...
        width: 100%; height: 40px; font-weight: bold; border-radius: 8px;
    }
