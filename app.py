...
def get_btdr_data():
    ...
    # --- 换手率精准计算逻辑 ---
    # 获取实时总成交量 (yf 提供的 fast_info 汇总通常比 1m 累加更准)
    try:
        realtime_v = tk.fast_info['last_volume']
    except:
        realtime_v = live_1m['Volume'].sum()
    
    # 流通盘 (Float) 动态校准: 优先取 API 数据，否则使用最新 1.18 亿基数
    try:
        float_shares = tk.info.get('floatShares', 118000000)
    except:
...
def get_ai_analysis(data_summary):
    ...
    try:
        ...
        prompt = f"""
        你是一名量化专家。审计 BTDR 盘面：
        - 现价: ${data_summary['curr_p']:.2f}, 支撑偏离: {dev_sup:.2f}%
        - 换手率: {data_summary['turnover']}, MA5: ${data_summary['ma5']:.2f}
        - 预测支撑/压力: ${data_summary['p_l_mid']:.2f} / ${data_summary['p_h_mid']:.2f}
        
        要求：1.空间定位；2.量价审计；3.操作指令[看多/看空/观望]及止损位。
        最后一行格式: SCORES: 动能=X, 支撑=X, 换手=X, 趋势=X
        """
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[{"role": "system", "content": "只讲干货，逻辑驱动。"}, {"role": "user", "content": prompt}]
        )
...
try:
    ...
    with c2:
        st.subheader("📍 场景预测目标")
        st.table(pd.DataFrame({"场景": ["中性回归", "乐观(+6%)", "悲观(-6%)"], 
                               "最高预测": [p_h_mid, p_h_mid*1.06, p_h_mid*0.94], 
                               "最低预测": [p_l_mid, p_l_mid*1.06, p_l_mid*0.94]}).style.format(precision=2))

    st.divider()

    # UI 模块 2: AI 研判
    if 'ai_scores' not in st.session_state: st.session_state.ai_scores = [50, 50, 50, 50]
    if 'ai_text' not in st.session_state: st.session_state.ai_text = "等待 AI 审计启动..."
    ...
    with col_text:
        ...
        if st.button("🚀 启动 AI 实战审计 (消耗额度)", use_container_width=True):
            data_sum = {'curr_p': curr_p, 'ma5': last_hist['5日均值'], 'turnover': f"{today_turnover:.2f}%", 
                        'p_l_mid': p_l_mid, 'p_h_mid': p_h_mid}
            with st.spinner('审计中...'):
                t, s = get_ai_analysis(data_sum)
                st.session_state.ai_text, st.session_state.ai_scores = t, s
        st.markdown(f"""<div style="background-color: rgba(0, 255, 255, 0.05); padding: 20px; border-radius: 12px; border-left: 6px solid #00CCCC; min-height: 250px;">{st.session_state.ai_text}</div>""", unsafe_allow_html=True)

    with col_radar:
        s = st.session_state.ai_scores
        fig_r = go.Figure(data=go.Scatterpolar(r=s + [s[0]], theta=['动能','支撑','换手','趋势','动能'], fill='toself', line=dict(color='#00CCCC')))
    ...
    fig_k.add_trace(go.Bar(x=p_df['label'], y=p_df['换手率_原始']*100, name="历史换手", marker_color='gray'), row=2, col=1)
