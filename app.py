...
def get_btdr_final_engine():
    tk = yf.Ticker("BTDR")
    info = tk.info
    # 锁定 1.18亿 基数确保换手率 4.73% 级别的精准度
    float_sh = 118000000 
...
def get_ai_audit_structured(p_curr, p_ma5, turnover, p_low, inst_cost):
    ...
    try:
        ...
        数据：现价 ${p_curr:.2f}, MA5 ${p_ma5:.2f}, 换手率 {turnover:.2f}%, 支撑偏离 {dev_sup:.2f}%, 机构成本位 ${inst_cost:.2f}。
        
        【输出指令】：
        1. 严禁简写，保留所有详实的逻辑分析。
        2. 必须严格按照以下结构输出：
           ## 1. 空间定位
           (分析现价与 MA5、回归支撑的关系，评估安全边际，加粗核心位)
           ## 2. 量价审计
           (分析当前换手率意图，是缩量阴跌还是主力吸筹，加粗关键信号)
           ## 3. 核心结论
           (给出最终操作建议：**持币观望/分批买入/逢高止盈**，并明确**参考止损价**)
        3. 全文必须是黑色字体，重点内容用 **双星号加粗**。
        """
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[{"role": "system", "content": "实战干货逻辑，重点突出。"}, {"role": "user", "content": prompt}]
        )
        return response.choices[0].message.content.strip()
    except: return "❌ 审计接口繁忙，请点击刷新按钮重试。"

# --- 3. UI 展示 ---
st.markdown("""<style> .main { background-color: #FFFFFF !important; } .stMarkdown, p, li, h2, h3, span { color: #000 !important; } 
    ...
    div.stButton > button { background-color: #FFF; color: #0044BB; border: 2px solid #0044BB; width: 100%; height: 42px; font-weight: bold; border-radius: 8px; }
    div.stButton > button:hover { background-color: #0044BB; color: #FFF; } </style>""", unsafe_allow_html=True)

try:
    hist_df, float_sh, reg, rt_v, inst_cost = get_btdr_final_engine()
    ...
    with c2:
        ...
        st.table(pd.DataFrame({"回归场景": ["上限压力位", "下限支撑位"], "预测价格": [p_h_pred, p_l_pred]}).style.format(precision=2))

    st.divider()

    # UI 2: AI 解析区 (单列全宽，移除雷达)
    st.subheader("🤖 AI 硬核形态研判 (结构化审计报告)")
    if st.button("🔄 刷新 AI 实时审计报告 (保留历史缓存)"):
        st.cache_data.clear()
        st.rerun()
        
    report = get_ai_audit_structured(curr_p, last_h['MA5'], today_to, p_l_pred, inst_cost)
    st.markdown(report)

    st.divider()

    # UI 3: K线与换手率
    st.subheader("🕒 实时监控主图 (阳红阴绿 + MA5红线)")
    ...
    fig_k.update_layout(height=550, xaxis_rangeslider_visible=False, template="plotly_white", margin=dict(l=10, r=10, t=10, b=10))
...
except Exception as e: st.error(f"终端运行异常: {e}")
