...
# 1. 基础配置 & 缓存清理 (最优先执行)
# ==========================================
st.set_page_config(page_title="BTDR Pilot v10.1", layout="centered")

# 启动时清理缓存
if 'init_v101' not in st.session_state:
    try:
        cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "yfinance")
        if os.path.exists(cache_dir): shutil.rmtree(cache_dir)
    except: pass
    st.session_state.clear()
    st.session_state['init_v101'] = True
...
def fetch_data_v101():
    ...
    # 默认因子
    factors = {
        "vwap": 10.0, "adx": 20.0, "regime": "Neutral", "beta_btc": 1.5, "beta_qqq": 1.2, "rsi": 50, "vol_base": 0.05
    }
    fng = 50
    ...
    try:
        ...
        if len(idx) > 20:
            ...
            ch = np.cov(x, yh)
            bh = safe_float(ch[0,1]/ch[0,0], 0.7)
            
            cl = np.cov(x, yl)
            bl = safe_float(cl[0,1]/cl[0,0], 0.9)
...
def dashboard():
    # 1. 抓取数据
    quotes, fng_val, model, factors = fetch_data_v101()
    ...
    # 3. 状态栏
    st.markdown(f"<div class='time-bar'>美东 {now_str} &nbsp;|&nbsp; 状态: <span style='background:{badge_c};color:white;padding:1px 3px;border-radius:3px;font-size:0.6rem'>{factors['regime']}</span> &nbsp;|&nbsp; 引擎: v10.1 (Stable)</div>", unsafe_allow_html=True)
    ...
    st.caption(f"Engine: v10.1 Final | Drift: {drift*100:.2f}% | Vol: {vol*100:.1f}%")

# ==========================================
# 6. 执行 (最后一步)
# ==========================================
if __name__ == "__main__":
    st.markdown("### ⚡ BTDR 领航员 v10.1")
    dashboard()
